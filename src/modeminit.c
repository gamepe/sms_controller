		...
		if (DEVICE.send_handshake_select)
		{
			...
			while (n > 0)
			{
				...
				if (got < 0)
				{
					...
					if (errno != EAGAIN)
					{
						printf("write_to_modem: error %d: %s", errno, strerror(errno));

						return 0;
					}

					printf("write_to_modem: device busy, waiting");
		...
		else
		{
			...
			if ((tio.c_cflag & CRTSCTS))
			{
				...
				while (!(status & TIOCM_CTS))
				{
					...
					{
						if (print_error)
							printf("Modem is not clear to send.\n");
						else
						{
							printf("Modem is not clear to send\n");
			...
			if (DEVICE.send_delay < 1)
			{
				...
				{
					printf("[x]Could not send string, cause: %s\n", strerror(errno));
					//alarm_handler0(LOG_ERR, tb);
					return 0;
				}
			...
			else
			{
				for(x=0;(size_t)x<strlen(command);x++)
				{
					if (write(modem_handle, command +x, 1) < 1)
					{
						...
						{
							printf("Could not send character %c, cause: %s", command[x], strerror(errno));
							//alarm_handler0(LOG_ERR, tb);
						}
						return 0;
					}
...
int put_command0(char *command, char *answer, int max, int timeout_count, char *expect, int silent)
{
	...
	if (expect && expect[0])
	{
		if (regcomp(&re, expect, REG_EXTENDED|REG_NOSUB) != 0)
		{
			...
			printf( "Programming error: Expected answer %s is not a valid regepr", expect);
			exit(1);
		}
		regex_allocated = 1;
	}
	...
	{
		if (!silent)
			printf( "Command is sent, waiting for the answer\n");
		...
		if (got_timeout)
		{
			...
			if (expect && expect[0])
				printf( "put_command expected %s, timeout occurred. %i.\n", expect, put_command_timeouts);
		}
...
void setmodemparams()
{
	...
	default:
		printf( "Baudrate %d not supported, using 19200\n", baudrate);
		baudrate = B19200;
...
int initmodem(char *new_smsc, int receiving)
{
	...
	// -----------------------------------------------------------------------------------------------
	printf( "Checking if modem is ready\n");

	DEVICE.needs_wakeup_at=1;;
	...
	if (!strstr(answer,"OK"))
	{
		...
		p = get_gsm_error(answer);
		// 3.1.5:
		printf(("Modem is not ready to answer commands%s%s (Timeouts: %i)\n",(*p)? ", " : "", p, put_command_timeouts));
	...
	if (DEVICE.pre_init > 0)
	{
		...
		if (!strstr(answer,"OK")) {
			printf( "Modem did not accept the pre-init string\n");
		}
	}
	...
	if (strcasecmp(DEVICE.pin, "ignore"))
	{
		...
		printf( "Checking if modem needs PIN");
		...
		if (strstr(answer,"+CPIN: SIM PIN") && !strstr(answer, "PIN2"))
		{
			...
			if (DEVICE.pin[0] == 0){
				...
				printf("Modem needs PIN, but it's not defined for this modem");

			}
			else
			{
				printf("Modem needs PIN, entering PIN...");
				...
				if (strstr(answer,"+CPIN: SIM PIN") && !strstr(answer, "PIN2"))
				{
					printf("Modem did not accept this PIN");
					//alarm_handler0(LOG_ERR, tb);
		...
		{
			printf("Your SIM is locked. Unlock it manually");
		...
		if (!strstr(answer, "+CPIN: READY"))
		{
			...
			//printf("PIN handling: expected READY, modem answered %s%s%s",change_crlf(cut_emptylines(cutspaces(answer)), ' '), (*p)? ", " : "", p));
			//alarm_handler0(LOG_CRIT, tb);
			// abnormal_termination(0);
		}
	...
	if (DEVICE.initstring[0])
	{
		...
		if (strstr(answer,"OK")==0)
		{
			// 3.1: more detailed error message:
			p = get_gsm_error(answer);
			printf(("Modem did not accept the init string%s%s", (*p)? ", " : "", p));
			// alarm_handler0(LOG_ERR, tb);
			return 3;
		}
	}
	...
	if (DEVICE.identity[0] == 0)
	{
		...
		if (!strstr(DEVICE.identity, "ERROR"))
		{
			...
			printf( "IMSI: %s", DEVICE.identity);
		}
		else{
			printf( "IMSI/CGSN not supported");
		}
	}
	...
	if (DEVICE.report_device_details)
	{
		...
		printf( "## Start of device details");
...
int openmodem()
{
	...
	while ((modem_handle = open(DEVICE.device, O_RDWR | O_NOCTTY | O_NONBLOCK)) < 0)
	{
		...
		printf ("Couldn't open serial port %s, error: %s, waiting %i sec.\n", DEVICE.device, strerror(errno), DEVICE.device_open_errorsleeptime);
	...
	if (modem_handle < 0)
	{
		if (modem_handle == -1)
		{
			printf("Cannot open serial connection\n ");
			//alarm_handler0(LOG_ERR, tb);
		}
		return -1;
	}
